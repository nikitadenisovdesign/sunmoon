/* ---------------------------------------------------------------
   SUNMOON — Pattern engine
   Procedural mirrored gradient strips on canvas.
   Auto-inits any <canvas data-pattern> element.
   Per-canvas overrides via data-pattern-config='{...}' (JSON).
--------------------------------------------------------------- */
...
(function () {
  ...
  /* Shared noise texture — generated once and reused across all instances */
  const NC = (() => {
  ...
  function create(cv, opts) {
    ...
    function makeGrad(g, x, y, w, h, deg) {
      ...
      STOPS.forEach(([p, col]) => gd.addColorStop(p, col));
      return gd;
    }
  ...
  document.querySelectorAll('[data-pattern]').forEach((el) => {
    let cfg = {};
    const raw = el.getAttribute('data-pattern-config');
    if (raw) {
